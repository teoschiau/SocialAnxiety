using UnityEngine;
using System.Collections.Generic;

public class Presentation_CrowdManager: MonoBehaviour
{
    [Header("Setup")]
    [Tooltip("The parent object that holds all the sitting NPCs as children.")]
    public Transform crowdParent; 

    private List<GameObject> allAudienceMembers = new List<GameObject>();

    void Awake()
    {
        // 1. Find all potential audience members
        if (crowdParent != null)
        {
            foreach (Transform child in crowdParent)
            {
                allAudienceMembers.Add(child.gameObject);
                // Ensure everyone is hidden at start
                child.gameObject.SetActive(false); 
            }
        }
        else
        {
            Debug.LogError("Presentation_CrowdManager: Crowd Parent is not assigned!");
        }
    }
    
    public void SetupAudience(int count)
    {
        ShuffleList(allAudienceMembers);

        int activeCount = 0;

        for (int i = 0; i < allAudienceMembers.Count; i++)
        {
            if (activeCount < count)
            {
                allAudienceMembers[i].SetActive(true);
                activeCount++;
            }
            else
            {
                allAudienceMembers[i].SetActive(false);
            }
        }
    }
    
    void ShuffleList<T>(List<T> list)
    {
        int n = list.Count;
        while (n > 1)
        {
            n--;
            int k = Random.Range(0, n + 1);
            T value = list[k];
            list[k] = list[n];
            list[n] = value;
        }
    }
    
    public GameObject GetRandomActiveMember() { 
        List<GameObject> activeMembers = allAudienceMembers.FindAll(member => member.activeInHierarchy);
        if (activeMembers.Count == 0) return null;
        return activeMembers[Random.Range(0, activeMembers.Count)];
    }
}